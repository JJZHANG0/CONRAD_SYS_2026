...
class LeanCanvasView(APIView):
    ...
    def patch(self, request, team_id):
        ...
        def save():
            canvas, _ = run_with_db_retry(
                lambda: LeanCanvas.objects.get_or_create(team=team)
            )
...
class LeanCanvasReviewView(APIView):
    """Operations-only: mark BMC modules as pass / fail."""
            ...
            try:
                def persist_review():
                    with transaction.atomic():
                        ...
                        canvas = LeanCanvas.objects.select_for_update().get(
                            pk=canvas.pk
                        )
                        canvas.field_reviews = merge_field_review(
                            canvas.field_reviews, field, review_status, BMC_FIELDS
                        )
                        canvas.save(
                            update_fields=["field_reviews", "updated_at"]
                        )
                        return LeanCanvasSerializer(canvas).data
